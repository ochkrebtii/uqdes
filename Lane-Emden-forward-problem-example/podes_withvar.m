function [evalgrid,u_sample,uvar] = podes_withvar(tspan,nsolves,Nknots,lambda,alpha,odefn,x0,pars,evalgrid)
...
u_sample        = repmat(repmat(x0,length(evalgrid),1),[1,1,B]);
...
for k = 1:N
    ...
    if k<N
        ...
        uvar                        = QQ1d(evalgrid,evalgrid,lambda,a,b)./alpha - qrbeval*(RQ1d(t(1:k),evalgrid,lambda,a,b)./alpha);
        for bb = 1:B
            u_sample(:,:,bb)         = repmat(u(1,:,bb),length(evalgrid),1) + qrbeval*Du(1:k,:,bb);
        end
